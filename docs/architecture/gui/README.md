...
|---|---|
| `MenuService` | Tracks which menu (if any) each player has open. Owns scroll state separately from open-state lifecycle. |
...
| `SkillsMenu` | Static builder that opens the skill tree at the player's current scroll offset. |
| `SkillTreeViewport` | Renders nodes, connections, and scroll controls into a 54-slot inventory. |
| `SkillTreeLayout` | Immutable coordinates of all skill nodes and connection paths. |
...
- `MenuService.skillScroll`: skill-tree scroll offset per player.
